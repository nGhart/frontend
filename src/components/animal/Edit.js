...
const EditAnimal = ({ animalData, handleClose }) => {
  const store = animalStore();
  const [show, setShow] = useState(true); // Show modal by default

  useEffect(() => {
    setShow(true); // Show the modal when it receives new data
  }, [animalData]);
  ...
  return (
    ...
    //     {/* Use the animalData prop to pre-fill the form fields */}
